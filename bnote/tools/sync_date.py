...
class SyncDate(threading.Thread):
    ...
    def run(self):
        ...
        if self.date:
            self.function(year=int(self.date['year']), month=int(self.date['month']), day=int(self.date['day']), hour=int(self.date['hour']), minute=int(self.date['minute']), second=int(self.date['second']))
