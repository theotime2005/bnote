...
class UiFileManagerTools:
        ...
        if os.path.exists(filename):
            if os.path.isdir(filename):
                if str(filename).startswith(str(Trash.get_trash_path())):
                    ...
                    return "".join((_("d "), self._convert_file_to_braille_text(braille_type, filename.name)))
    ...
    def friendly_file_name(filename, replace_backup=False):
        ...
        friendly_name = friendly_name.replace(str(FileManager.get_backup_path()), _("backups"))
        friendly_name = friendly_name.replace(str(FileManager.get_usb_flash_drive_path()), _("usb flash drives"))
        # Alway try to replace document after the documents sub folder (like symbolic link
        # on trash / bluetooth / backup / usb flash drive that are located inside documents folders.)
        friendly_name = friendly_name.replace(str(FileManager.get_root_path()), _("my Documents"))
