...
class UiFileManagerTools:
    ...
    def _ui_file_name(self, braille_type, filename):
        ...
        if os.path.exists(filename):
            if os.path.isdir(filename):
                if filename != Trash.get_trash_path() and str(filename).startswith(
                    str(Trash.get_trash_path())
                ):
                    log.debug("trash")
                    original_path = Trash.original_file(filename)
                    deletion_date = Trash.deletion_date(filename)
                    msg, pos = self._convert_to_braille_text(
                        braille_type,
                        _("deleted on {}").format(
                            deletion_date.strftime(_("%A %d %B %Y %H:%M:%S"))
                        ),
                    )
                    friendly_name = UiFileManagerTools.friendly_file_name(
                        original_path, True
                    )
                    return "".join(
                        (
                            _("d "),
                            original_path.name,
                            self._convert_file_to_braille_text(
                                braille_type, " ({}) ".format(friendly_name)
                            ),
                            msg,
                        )
                    )
                    ...
                    return "".join(
                        (
                            ...
                            self._convert_file_to_braille_text(
                                braille_type,
                                " ({}) ".format(
                                    UiFileManagerTools.friendly_file_name(
                                        original_path, True
                                    )
                                ),
                            ),
    ...
    def _convert_to_braille_text(self, braille_type, text, pos=-1) -> (str, int):
        if braille_type == "grade1":
            if pos >= 0:
                (text_grade, index1_origin, index_text, pos) = (
                    BnoteApp.lou.text_to_grade1(text, pos)
                )
            else:
                (text_grade, index1_origin, index_text, pos) = (
            ...
            if pos >= 0:
                (text_grade, index1_origin, index_text, pos) = (
                    BnoteApp.lou.text_to_grade2(text, pos)
                )
            else:
                (text_grade, index1_origin, index_text, pos) = (
    ...
    def _convert_file_to_braille_text(self, braille_type, filename):
        ...
        if len(parts) > 1:
            ...
            for part in parts[0 : len(parts) - 1]:
                part_braille_text, pos = self._convert_to_braille_text(
                    braille_type, part
                )
                if braille_text:
        ...
        if braille_text:
            braille_text = "/".join(
                [
                    braille_text,
                    self._convert_file_name_to_braille_text(braille_type, part),
                ]
            )
        else:
    ...
    def _convert_file_name_to_braille(self, braille_type, file) -> (str, str):
        ...
        text_name, braille_name, pos = BnoteApp.lou.convert_to_braille(
            braille_type, file_name
        )
        text_suffix, braille_suffix, pos = self._convert_suffix_to_braille(
            braille_type, file_suffix
        )
        ...
        if braille_suffix != "":
            return braille_name, "".join(
                [braille_name, braille_separator, braille_suffix]
            )
        else:
    ...
    def _convert_file_name_to_braille_text(self, braille_type, file) -> str:
        ...
        text_suffix, pos = self._convert_suffix_to_braille_text(
            braille_type, file_suffix
        )
    ...
    def _convert_suffix_to_text(self, braille_type, braille, pos=-1):
        ...
        if braille_type == "grade1" or braille_type == "grade2":
            if pos >= 0:
                (text_grade, index1_origin, index_text, pos) = (
                    BnoteApp.lou.grade1_to_text(braille, pos)
                )
            else:
                (text_grade, index1_origin, index_text, pos) = (
                    BnoteApp.lou.grade1_to_text(braille)
                )
                pos = -1
            log.debug(
                "grade1:<{}>, {}, {}, {}".format(
    ...
    def friendly_file_name(filename, replace_backup=False):
        ...
        log.debug(f"filename={filename}")
        friendly_name = str(filename).replace(
            str(FileManager.get_tmp_path()) + os.path.sep, ""
        )
        if replace_backup:
            friendly_name = friendly_name.replace(
                str(FileManager.get_backup_path()) + os.path.sep, ""
            )

        friendly_name = friendly_name.replace(str(Trash.get_trash_path()), _("trash"))
        friendly_name = friendly_name.replace(
            str(FileManager.get_bluetooth_path()), _("bluetooth")
        )
        friendly_name = friendly_name.replace(
            str(FileManager.get_backup_path()), _("my backups")
        )
        friendly_name = friendly_name.replace(
            str(FileManager.get_usb_flash_drive_path()), _("usb flash drives")
        )
        friendly_name = friendly_name.replace(
            str(FileManager.get_documents_path()), _("my Documents")
        )
        friendly_name = friendly_name.replace(
            str(FileManager.get_crash_path()), _("crash")
        )
        friendly_name = friendly_name.replace(
            str(FileManager.get_settings_path()), _("settings")
        )
        ...
        friendly_name = friendly_name.replace(
            str(FileManager.get_main_folder_path()), _("bnote")
        )
        ...
        if os.path.exists(full_file_name):
            if os.path.isfile(full_file_name):
                ...
                if size < 1000:
                    ...
                    return "".join(
                        (" ({:.1f} ".format(size / 1000000000), _("Gb"), ")")
                    )
                else:
                    return "".join((" ({} ".format(size), _("bytes"), ")"))
