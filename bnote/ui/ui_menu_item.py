...
class UiMenuItem(UiObject):
    ...
    def __init__(self, name, action, action_param=None, is_hide=False, shortcut_modifier=None, shortcut_key=None):
        kwargs = {
        ...
        super(UiMenuItem, self).__init__(**kwargs)
    ...
    def set_name(self, name):
        ...
        if Settings().data['system']['shortcuts_visible'] and self._shortcut_modifier is not None:
            modifier_str = ""
            ...
            if isinstance(self._shortcut_key, Keyboard.BrailleFunction):
                shortcut_str = Keyboard().BrailleFunction.get_bramigraph_label(self._shortcut_key)
            else:
