...
class WriteBrfFile(threading.Thread):
    def __init__(self, get_line, full_file_name, language, braille_type, line_length, page_line, max_page, first_page, braille_table, on_end):
    ...
    def get_title(self, first_page):
        ...
        for line in page:
            ...
            self.first_page.append(Lou(self.language).convert_to_braille(self.braille_type, new_line)[1])
    ...
    def import_and_convert_file(self):
        ...
        while True:
            ...
            file_convert.append(Lou(self.language).convert_to_braille(self.braille_type, line_convert)[1])
    ...
    def __get_space(self, line, page_number):
        ...
        for _ in range(space_number):
            spaces+=chr(10240)
        spaces+=Lou(self.language).convert_to_braille(self.braille_type, str(page_number))[1]
        return spaces
    ...
    def _cut_document(self, content):
        ...
        if len(lines)>self.page_line:
            while lines:
                ...
                if page_count>=2:
                    new_page[0]+="{}".format(self.__get_space(new_page[0],page_count))
    ...
    def _get_volume(self):
        if len(self.page_document)>self.max_page:
            ...
            while self.page_document:
                ...
                volume_title=Lou(self.language).convert_to_braille(self.braille_type, "\n{}\n".format(volume_number))[1]
                volume_to_insert=self._cut_document(self.first_page+[volume_title]+self.__get_lines(len(self.first_page+[volume_title])))
