...
from .colored_log import ColoredLogger, READ_FILE_LOG
...
class ReadFile(threading.Thread):
    ...
    def create_save_file_name(full_file_name):
        ...
        if path.exists(new_name):
            # File already exists.
            raise BadNewFileName(_('file exists {}, probably because the original file has already been converted. '
                                   'Please open the converted file or rename/delete it if you want to try again this '
                                   'conversion.'.format(new_name)))
        else:
            return new_name
