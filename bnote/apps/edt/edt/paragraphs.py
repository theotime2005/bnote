...
from .colored_log import ColoredLogger, EDITOR_PARAGRAPH_LOG
...
class Paragraphs:
    ...
    def __str__(self):
        text = "--------------\r\n"
        text += "Document of {} paragraph\r\n".format(len(self._paragraphs))
    ...
    def add(self, line):
        """
        Add a new paragraph to collection with a line of text converted into multi-line paragraph.
        this function is used to construct document model.
        line: text of a paragraph
        -> No return
        """
        self._paragraphs.append(Paragraph(self._remove_control_chars(line), self._width))

    def lines_count(self, paragraph_index=None):
        """
        Get the number of lines in the document when paragraph are justified until paragraph_index.
        ...
        if len(strings) == 1:
            ...
            for num, string in enumerate(strings):
                ...
                if num == 0:
                    ...
                    markers_saved = markers.extract(coo, Paragraph(after_string, self._width), len(after_string))
                    coo.line = new_pos.y
                    coo.column = new_pos.x
                    ...
                    # last line to insert
                    coo.paragraph += 1
                    self._paragraphs.insert(coo.paragraph, Paragraph(s, self._width))
                    # Update markers positions after paragraph insertion
                    markers.insert_paragraphs(coo, 1)
                    # inc offset order of all next paragraph
                    # Cette ligne a été introduite lors d'une comparaison avec le fichier équivalent de bsuite,
                    # elle ne correspond pas, là bas non plus, à une méthode existante ?
                    ...
                    self._paragraphs.insert(coo.paragraph, Paragraph(s + after_string, self._width))
                    # Update markers positions after paragraph insertion
                    markers.insert_paragraphs(coo, 1)
                    if after_string != "":
                        # Replace the markers situated in after_string area
