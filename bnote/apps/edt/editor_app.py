...
class EditorApp(EditorBaseApp):
            ...
            menu_item_list=[
                ui.UiMenuBar(
                    ...
                    menu_item_list=[
                        ...
                        ui.UiMenuItem(
                            name=_("&export to .brf"), action=self._exec_export_brf
                        ),
                        ui.UiMenuItem(
                            name=_("send se&lection to"), action=self._exec_send_to
                        ),
                ...
                ui.UiMenuBar(
                    name=_("&insert"),
                    action=EditorApp.INSERT_MENU_ID,
                    menu_item_list=[
                        ui.UiMenuItem(
                            name=_("&result"), action=self._exec_insert_result
                        ),
                        ui.UiMenuItem(name=_("&date"), action=self._exec_insert_date),
                        ui.UiMenuItem(
                            name=_("&signature"), action=self._exec_insert_signature
                        ),
                    ],
                ),
                ...
                ui.UiMenuBar(
                    name=_("&math"),
                    action=EditorApp.MATH_MENU_ID,
                    menu_item_list=[
                        ui.UiMenuItem(
                            name=_("&line evaluation"),
                            action=self._exec_math_line_eval,
                            shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL,
                            shortcut_key="M",
                        ),
                        ui.UiMenuItem(
                            name=_("&bloc evaluation"),
                            action=self._exec_math_bloc_eval,
                            shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL,
                            shortcut_key="T",
                        ),
                    ],
        ...
        if not self._dialog_is_reading_file():
            self._current_dialog = ui.UiDialogBox(
                ...
                item_list=[
                    ...
                    ui.UiListBox(
                        name=_("&braille type"),
                        value=("braille", [_("grade 1"), _("grade 2")]),
                        current_index=0,
                    ),
                    ...
                    ui.UiMultiLinesBox(
                        name=_("click &here to enter the first page"),
                        value=("first_page", ""),
                    ),
    ...
    def _exec_valid_export_brf(self, force=False):
        ...
        export = editor.WriteBrfFile(
            self._get_line,
            str(self._original_file_name),
            braille_language,
            braille_type,
            line_length,
            page_line,
    ...
    def _exec_end_export(self, activity):
        messages = {
            "failed": _(
                "the value of character per line, line per page and page per file must be an integer."
            ),
            "success": _("exporting was successful."),
            "exist": _(
                "an element of the same name already exists, remove it and try again."
            ),
        }
        self._current_dialog = ui.UiInfoDialogBox(
            message=messages[activity], action=self._exec_cancel_dialog
        )
    ...
    def _exec_send_to(self):
        if not self._dialog_is_reading_file():
            if not len(BnoteApp.bluetooth_devices):
                ...
                self._current_dialog = ui.UiInfoDialogBox(
                    message=_("you must select text to send"),
            ...
            self._current_dialog = ui.UiDialogBox(
                ...
                item_list=[
                    ui.UiListBox(
                        name=_("&device"),
                        value=("device_list", bluetooth_list),
                        current_index=0,
                    ),
    ...
    def _exec_valid_send_to(self):
        ...
        self._current_dialog = ui.UiMessageDialogBox(
            ...
            buttons=[
                ui.UiButton(
                    name=_("&ok"), action=self._send_to, action_param={"id_": id_}
                ),
                ui.UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
            ],
            action_cancelable=self._exec_cancel_dialog,
    ...
    def _exec_translate(self):
        ...
        self._put_in_function_queue(
            FunctionId.FUNCTION_OPEN_TRANSLATOR, **{"source": self.editor.selection()}
        )
        ...
        try:
            ...
            if math_expression:
                ...
                if mathlib.isnan(res):
                    raise math.MathException(
                        start,
                        math.MathException.ErrorCode.NOT_EVALUABLE,
                        _("nan value"),
                    )
                if mathlib.isinf(res):
                    raise math.MathException(
                        start,
                        math.MathException.ErrorCode.INFINITE_VALUE,
                        _("infinite value"),
                    )
                ...
                self._current_dialog = ui.UiInfoDialogBox(
                    message=msg, action=self._exec_cancel_dialog
                )
            else:
                self._current_dialog = ui.UiInfoDialogBox(
                    message=_("Error forgotten formula start marks"),
                    action=self._exec_cancel_dialog,
                )
            ...
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("Error:{} at pos:{}").format(er.__str__(), er.pos),
                action=self._exec_cancel_dialog,
            )
            ...
            try:
                while continue_loop:
                    ...
                    if math_expression:
                        ...
                        # Next line
                        continue_loop = self.editor.function(
                            editor.Editor.Functions.MOVE_DOWN,
                            **{"shift": False, "ctrl": False},
                        )
                    else:
    ...
    def _exec_insert_result(self):
        ...
        """
        line = "r"
        parser = math.MathParser(
            math.MathTextTable, Settings().data["math"]["angle"], BnoteApp.lou
        )
        if parser:
            ...
            if math_expression:
                try:
                    ...
                    self._current_dialog = ui.UiInfoDialogBox(
                        message=_("no result"), action=self._exec_cancel_dialog
                    )
    ...
    def _exec_insert_signature(self):
        ...
        try:
            signature_file = open(
                FileManager.get_root_path() / Path("signature.txt"), "r"
            )
        ...
        if signature:
            with self.lock:
                self.editor.function(
                    editor.Editor.Functions.PUT_STRING, **{"text": signature}
                )
            return True, None
        else:
            self._current_dialog = ui.UiInfoDialogBox(
