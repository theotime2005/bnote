...
class EditorBaseApp(BnoteApp):
    ...
    def __init__(self, put_in_function_queue, file_name=None, language=None):
        super(EditorBaseApp, self).__init__(put_in_function_queue)
        ...
        if not file_name:
            ...
            if not self.read_context():
                return
            # Just for test, Editor is always opened with an associated filename, at least an empty file.
            # lines = ["0123456789", "1234567890", "2345678901", "3456789012"]
            # self.editor = Editor(self.editor_line_length(), lines)
        else:
            # Read the  file
    ...
    def create_sub_menu_edit(self):
        return UiMenuBar(
            name=_("&edit"),
            menu_item_list=[
                UiMenuItem(name=_("&undo"), action=self._exec_undo,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='Z'),
                UiMenuItem(name=_("&redo"), action=self._exec_redo,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='Y'),
                UiMenuBar(
                    name=_("&selection"),
                    menu_item_list=[
                        UiMenuItem(name=_("&start selection"), action=self._exec_start_selection,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                                   shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F8),
                        UiMenuItem(name=_("&end selection"), action=self._exec_end_selection, is_hide=True,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                                   shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_ESCAPE),
                        UiMenuItem(name=_("to the s&tart of doc"), action=self._exec_select_beginning),
                        UiMenuItem(name=_("to the e&nd of doc"), action=self._exec_select_ending),
                ...
                UiMenuItem(name=_("curs&or"), action=self._exec_cursor),
                UiMenuItem(name=_("forward in grade2 braille"), action=self._toggle_grade2),
            ])
    ...
    def create_sub_menu_find(self):
        return UiMenuBar(
            name=_("fi&nd"),
            menu_item_list=[
                UiMenuItem(name=_("fi&nd"), action=self._exec_find,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='F'),
                UiMenuItem(name=_("&replace"), action=self._exec_replace,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='H'),
                UiMenuItem(name=_("ne&xt"), action=self._exec_next,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                           shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F3),
                UiMenuItem(name=_("replace and n&ext"), action=self._exec_replace_and_next,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                           shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F4),
                UiMenuItem(name=_("&previous"), action=self._exec_previous,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_SHIFT,
                           shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F3),
                UiMenuItem(name=_("replace and pre&vious"), action=self._exec_replace_and_previous,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_SHIFT,
                           shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F4),
                UiMenuItem(name=_("replace &all"), action=self._exec_replace_all,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
    ...
    def create_sub_menu_goto(self):
        return UiMenuBar(
            name=_("&goto"),
            menu_item_list=[
                UiMenuItem(name=_("&beginning of document"), action=self._exec_beginning_document),
                UiMenuItem(name=_("&end of document"), action=self._exec_end_document),
                UiMenuItem(name=_("&previous paragraphe"), action=self._exec_preview_paragraphe),
                UiMenuItem(name=_("&next paragraphe"), action=self._exec_next_paragraphe),
                ])
    ...
    def create_sub_menu_bookmark(self):
        return UiMenuBar(
            name=_("&bookmark"),
            menu_item_list=[
                UiMenuItem(name=_("&insert/delete"), action=self._exec_marker_insert_delete,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL,
                           shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F2),
                UiMenuItem(name=_("ne&xt"), action=self._exec_marker_next,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                           shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F2),
                UiMenuItem(name=_("&previous"), action=self._exec_marker_previous,
                           shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_SHIFT,
                           shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_F2),
                UiMenuItem(name=_("clear &all"), action=self._exec_marker_clear_all,
        ...
        if error is None:
            if self._file_name is not None and path.exists(doc_file):
                ...
                with self.lock:
                    ...
                    write_specific_file.write_specific_file(self.editor.markers(), self.editor.caret())
                    self._shutdown_processing = False
        else:
    ...
    def read_file(self, file_name, language, read_file_ended, sheet_name=None):
        ...
        (md5_from_specific, self.caret_from_specific,
         self.markers_from_specific) = read_specific_file.read_specific_file()
        if md5_from_specific is not None:
            log.warning("Specific file found")
            md5_file = editor.checksum(file_name)
            if md5_from_specific != md5_file:
                # The file to open is not corresponding to the specific file
                self.caret_from_specific = None
                self.markers_from_specific = None
            else:
                log.info("caret from specific file: {}".format(self.markers_from_specific))
                log.info("markers from specific file: {}".format(self.markers_from_specific))
        else:
        ...
        self._current_dialog = UiInfoDialogBox(message=_("reading..."))
    ...
    def read_file_add_line(self, line):
        if self._killed:
        ...
        with self.lock:
            if not self.editor:
                ...
                self.editor.add_line(self._transform_text_to_braille(line))
    ...
    def _exec_close(self):
        log.warning("Close without saving")
        if self.editor.is_modified():
            # Ask confirmation.
            self._current_dialog = UiMessageDialogBox(
                ...
                buttons=[
                    UiButton(name=_("&yes"), action=self._exec_close_yes_dialog),
                    UiButton(name=_("&no"), action=self._exec_close_no_dialog),
    ...
    def _exec_cleanup(self):
        ...
        self._current_dialog = UiInfoDialogBox(message=_("cleanup..."))
        self.wait_dialog = True
    ...
    def _exec_find(self):
        log.info("Dialogbox Find")
        # Do a copy to allow cancel without degrading find's parameters
        self._current_dialog = UiDialogBox(
            name=_("find"),
            item_list=[
                UiEditBox(name=_("find"),
                          value=("find", self._replace_parameters.edit_seq)
                          ),
                UiButton(name=_("&next"), action=self._exec_valid_next_find_dialog),
                UiButton(name=_("&previous"), action=self._exec_valid_previous_find_dialog),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
                UiCheckBox(
    ...
    def _exec_replace(self):
        ...
        self._current_dialog = UiDialogBox(
            ...
            item_list=[
                ...
                UiEditBox(name=_("replace"),
                          value=("replace", self._replace_parameters.replace_seq)
                          ),
                UiButton(name=_("&next"), action=self._exec_valid_next_find_dialog),
                UiButton(name=_("replace and n&ext"), action=self._exec_valid_next_replace_dialog),
                UiButton(name=_("&previous"), action=self._exec_valid_previous_find_dialog),
                UiButton(name=_("replace and p&revious"), action=self._exec_valid_previous_replace_dialog),
                UiButton(name=_("replace &all"), action=self._exec_valid_replace_all_dialog),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
                UiCheckBox(
                    name=_("&ignore upper case"),
                    value=("ignore_case", self._replace_parameters.is_ignore_case())),
    ...
    def _exec_cancel_dialog(self):
        # Close dialog box. => Nothing to do, an action from a dialog box closes it.
        pass
    ...
    def _select_sheet(self):
        self._current_dialog = UiDialogBox(
            name=_("editor"),
            item_list=[
                UiListBox(name=_("&sheet"), value=('sheet_name', self.last_error.sheet_list)),
                UiButton(name=_("&ok"), action=self._exec_ok_select_sheet_dialog),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
            ],
            action_cancelable=self._exec_cancel_dialog
        )
    ...
    def input_function(self, *args, **kwargs) -> bool:
        ...
        if function_id == FunctionId.FUNCTION_SAVE_AND_DELETE_ORIGINAL:
            ...
            self._current_dialog = UiInfoDialogBox(message=text, action=self._exec_cancel_dialog)
            done = True
            ...
            if self.last_error:
                text = "{}".format(self.last_error)
            self._current_dialog = UiInfoDialogBox(message=text, action=self._exec_close)
            done = True
    ...
    def input_bramigraph(self, modifier, bramigraph) -> bool:
        ...
        if bramigraph == Keyboard.BrailleFunction.BRAMIGRAPH_ESCAPE:
            ...
            self.is_editing = True
        done = super(EditorBaseApp, self).input_bramigraph(modifier, bramigraph)
        if not done:
    ...
    def _toggle_grade2(self):
        log.info("forward display grade2")
        if self._is_forward_grade2:
    ...
    def on_close(self):
        ...
        if self._file_name is not None and path.exists(self._file_name):
            ...
            if file_extension:
                log.info("Save specific files for {}".format(self._file_name))
                with self.lock:
                    # Write specific file associated to the text file written (if this file exists).
