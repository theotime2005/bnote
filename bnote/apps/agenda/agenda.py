"""
Copyright (C) 2022 Theotime Berthod
...
class Editing:
    ...
    def export_file(self, name):
        path=os.listdir("/home/pi/.bnote/bnote-documents")
        for file in path:
            if file==name+".csv":
                return "exist"
        self.range_element()
        f = open("/home/pi/.bnote/bnote-documents/"+name+".csv", "w", encoding="utf-8")
    ...
    def verify_date(self, date):
        ...
        if int(date[0])<=0 or int(date[0])>31:
            ...
            return "month"
        if int(date[2])%400==0 and int(date[2])%4==0:
            if int(date[0])>=30:
                return "february"
        if int(date[2])%100==0 or int(date[1])==2:
            if int(date[1])==2 and int(date[0])>=29:
                return "bissextile"
        if int(date[1])==(4,6,9,11) and int(date[0])>=31:
            return "30 days"
        return True
    ...
    def range_element(self):
        ...
        for element in self.lst_not_treated:
            ...
            tmp.append([str(datetime.datetime(int(date[2]), int(date[1]), int(date[0]))),element[1],element[2],element[3]])
        tmp=sorted(tmp)
