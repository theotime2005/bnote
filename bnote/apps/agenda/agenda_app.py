...
class AgendaApp(BnoteApp):
    ...
    def __init__(self, put_in_function_queue):
        ...
        if Settings().data['agenda']['default_presentation'] != 'standard' and Settings().data['agenda'][
            'default_presentation'] != 'calendar':
    ...
    def error_agenda(self):
        ...
        self._current_dialog = ui.UiInfoDialogBox(
            message=_("the file has been correctly copy and the agenda has clear"), action=self._exec_cancel_dialog)
        ...
        for event in self.lst_treatment:
            if self.compare_date(event.date):
                if not number:
                    ...
                    self.lst.append(self.get_name_day(event.date) + " " + self.todo_dic[
                        event.todo] + "-" + event.subject + ": " + event.content)
                else:
                    value += 1
        if number:
            if value >= 1:
    ...
    def _select_mode(self):
        self.lst = [_("from today's ({})").format(self.standard_presentation(True)),
    ...
    def _exec_find_event(self, txt="", criter_index=0, only_not_done=False):
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiListBox(name=_("cri&terion"), value=("criter", [_("all"), _("subject"), _("content")]),
    ...
    def _exec_export_agenda(self, name="", path=None):
        ...
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiListBox(name=_("file &location"), value=("location", [_("my documents"), _("choose folder")]),
            ...
            self._current_dialog = ui.UiMessageDialogBox(
                ...
                buttons=[
                    ui.UiButton(name=_("&ok"), action=self._exec_add_event,
                                action_param={"date": date_compose, "subject": kwargs['subject'],
            ...
            self._current_dialog = ui.UiMessageDialogBox(
                ...
                buttons=[
                    ui.UiButton(name=_("&ok"), action=self._exec_modify_event,
                                action_param={"date": date_compose, "subject": kwargs['subject'],
                                              "content": kwargs['content'], "todo": kwargs['todo']}),
                    ui.UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
                ],
                action_cancelable=self._exec_cancel_dialog,
            )
    ...
    def _exec_valid_find_element(self):
        ...
        self._current_dialog = ui.UiInfoDialogBox(
            ...
            action_param={"txt": kwargs['string character'], "criter_index": dico_number[kwargs['criter']],
    ...
    def _exec_valid_import_file(self):
        ...
        if file.is_file():
            ...
            if importer == False:
                text_name = _("warning")
                text = _(
                    "the file {} does not comply with the application's convention. Refer to the documentation for more information.").format(
                    file)
                ...
                if importer == 1:
                    ...
                    text = _(
                        "the file was correctly imported, with the exception of {} events that already existed.").format(
                        importer)
            else:
                text_name = _("information")
    ...
    def _exec_valid_export(self, name, path=None):
        ...
        if exporter == True:
            ...
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("the file has been correctly exported in {} folder").format(path),
    ...
    def close_after_same_day(self):
        if Settings().data['agenda']['remember'] == "tomorrow" or Settings().data['agenda'][
    ...
    def get_name_day(self, date, complete=False):
        ...
        if complete == True:
            return name_day_dict[date_name] + " " + date_found[0] + " " + month_dict[date_found[1]] + " " + date_found[
                2]
        ...
        for idx, element in enumerate(self.lst_treatment):
            if todo and element.todo == "1":
                if element.subject.lower().find(text_to_find) == 0:
                    ...
                    self.lst.append(self.get_name_day(element.date) + " " + self.todo_dic[
                        element.todo] + ": " + element.subject + "-" + element.content)
    ...
    def __quick_search_move_call_back(self, text_to_find):
        ...
        for index, element in enumerate(lst):
            if self.filter == "mode":
                ...
                if element.subject.lower().find(
                        text_to_find) == 0 and index > self.index or element.content.lower().find(
                    text_to_find) == 0 and index > self.index:
                    self.index = index
                    self.set_data_line()
                    return True
