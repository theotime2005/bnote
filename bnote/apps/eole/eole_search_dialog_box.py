...
class EoleSearchDialogBox(ui.UiDialogBox):
    def __init__(
        self,
        is_eole_simplified_search,
        exec_show_summary,
        exec_download,
        exec_display_error_dialog,
        ...
        self._result_listbox = ui.UiListBox(
            name=_("result"), value=("result", list(self._result.keys()))
        )
        ...
        if self.__is_eole_simplified_search:
            item_list = [
                ui.UiListBox(
                    name=_("support"),
                    value=("support", list(self._support_value.keys())),
                ),
                ui.UiEditBox(
                    name=_("search for authors, titles and abstracts"),
                    value=("search", ""),
                ),
                ui.UiButton(
                    name=_("&search"), action=self._exec_search, is_auto_close=False
                ),
                self._result_listbox,
                ui.UiButton(
                    name=_("&summary"), action=exec_show_summary, is_auto_close=False
                ),
                ui.UiButton(
                    name=_("&download"), action=exec_download, is_auto_close=False
                ),
            ...
            item_list = [
                ...
                ui.UiListBox(
                    name=_("genre"), value=("genre", list(self._genre.keys()))
                ),
    ...
    def _exec_search(self):
        search, genre, author, title, age_range, support, editor, description = (
            None,
            None,
            None,
            None,
        ...
        if error_message is not None:
            ...
            if publication_count > MAX_PUBLICATION_COUNT:
                message = _(
                    "to many result (found {} and maxi is {})".format(
                        publication_count, MAX_PUBLICATION_COUNT
                    )
                )
        ...
        if "links" in current_data:
            for link in current_data["links"]:
                ...
                if "type" in link and link["type"] == "application/zip":
                    ...
                    if (
                        "href" in link
                        and "metadata" in current_data
                        and "archive" in current_data["metadata"]
                    ):
                        url_name = link["href"]
