...
class EoleApi(metaclass=SingletonMeta):
    ...
    def init_access_token_from_user_password(self, user_name, password) -> (bool, str):
        ...
        if self.__eole_access_token is None:
            self.__eole_access_token = EoleAccessToken(user_name, password, url_api=self.__url_api)
        else:
            self.__eole_access_token.set_user(user_name, password)
        access_token, error_success_message = self.__eole_access_token.acquire_access_token()
        if access_token:
            self.__token['access_token'] = access_token
            self.__session = OAuth2Session(token=self.__token)
    ...
    def download(self, url_name, file_name) -> (bool, str):
        ...
        if self.has_valid_token_access():
            ...
            if response.status_code == 200:
                ...
                return False, "server response = {} - {]".format(response.status_code, response.content.decode('utf-8'))
    ...
    def my_history(self) -> dict:
        if self.has_valid_token_access():
            response = self.__session.get('/'.join((self.__url_api, 'my_history')), timeout=TIMEOUT)
            print(f"{response.status_code=}")
    ...
    def search(self, search=None, genre=None, author=None, title=None, age_range=None, isbn=None, created_at_after=None,
               created_at_before=None, updated_at_after=None, updated_at_before=None, dewey=None,
               original_language=None, language=None, description=None, editor=None, notice_id=None, adaptation=None,
               support=None, duration_after=None, duration_before=None, auto_build_the_list=True, max_items=100) -> {}:
        ...
        extra_params = {'genre': genre, 'ageRange': age_range,
                        'isbn': isbn, 'createdAt[after]': created_at_after, 'createdAt[before]': created_at_before,
                        'updatedAt[after]': updated_at_after, 'updatedAt[before]': updated_at_before, 'dewey': dewey,
                        'originalLanguage': original_language, 'language': language, 'description': description,
                        'editor': editor, 'noticeId': notice_id, 'adaptation': adaptation, 'support': support,
                        'duration[after]': duration_after, 'duration[before]': duration_before}
        ...
        # Call the search API
        response = self.__session.get('/'.join((self.__url_api, 'search')), timeout=TIMEOUT, params=params)
        # print(f"{response.status_code=}")
        # print(f"{response.content=}")
        ...
        if response.status_code == 200:
            if auto_build_the_list:
                if 'metadata' in response_dict and 'numberOfItems' in response_dict['metadata']:
                    ...
                    if 0 < numbers_of_items <= max_items:
                        ...
                        publications = self.__extract_items('publications', response_dict)
                        # print(f"{publications=}")
                        response_dict = {'metadata': response_dict['metadata'], 'publications': publications}
    ...
    def get_genre(self) -> {}:
        # Call the genre API
        response = self.__session.get('/'.join((self.__url_api, 'choix-genre-moteur-de-recherche/genre')),
                                      timeout=TIMEOUT)
        # print(f"{response.status_code=}")
        # print(f"{response.content=}")
        ...
        if response.status_code == 200:
            if 'metadata' in response_dict and 'numberOfItems' in response_dict['metadata']:
                ...
                if 0 < numbers_of_items:
                    ...
                    # print(f"{publications=}")
                    return {'metadata': response_dict['metadata'], 'genres': navigations}
    ...
    if 'publications' in result:
        ...
        for link in wanted_publication['links']:
            ...
            if 'type' in link and link['type'] == "application/zip":
                ...
                success, message = EoleApi().download(link['href'], wanted_publication['metadata']['archive'])
