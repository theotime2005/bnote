...
class OperationApp(BnoteApp):
        ...
        return ui.UiMenuBar(
            ...
            menu_item_list=[
                ui.UiMenuBar(
                    ...
                    menu_item_list=[
                        ui.UiMenuItem(
                            name=_("&show to this table"), action=self._exec_show_score
                        ),
                        ui.UiMenuItem(
                            name=_("&delete to this table"),
    ...
    def _update_menu_items(self):
        ...
        if self.game:
            ...
            self._menu.rename_item(
                _("&change to table"), braille_type, self._exec_select_table
            )
        else:
            delete.hide()
            score.hide()
            self._menu.rename_item(
                _("&select table"), braille_type, self._exec_select_table
            )
        self.set_data_line()
    ...
    def _exec_what_is_result(self):
        self._current_dialog = ui.UiMessageDialogBox(
            name=_("information"),
            message=_("the result was {}").format(
    ...
    def _exec_select_table(self):
        ...
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ui.UiListBox(
                    name=_("&table to"), value=("table", lst_table), current_index=index
                ),
    ...
    def _exec_about(self):
        ...
        self._current_dialog = ui.UiMessageDialogBox(
            ...
            message=_("Application {} version {}. {} Theotime Berthod").format(
                _("math game"), app_version, "Copyright (C)2023"
            ),
            buttons=[
    ...
    def _exec_test_operation(self):
        ...
        if test:
            ...
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("good, the result is {}").format(
                    self.operation.lst_expression[self.expression]
                ),
            ...
            audio.file_play(
                str(
                    Path(self.get_apps_folder() / Path("braille_learning/warning.wav"))
                ),
                0,
            )
            self._current_dialog = ui.UiMessageDialogBox(
                ...
                buttons=[
                    ui.UiButton(name=_("&ok"), action=self._exec_cancel_dialog),
                    ui.UiButton(
                        name=_("&see the solution"), action=self._exec_what_is_result
                    ),
                ],
            )
    ...
    def _exec_show_score(self):
        if self.table == "":
            ...
            name = self.table
        self._current_dialog = ui.UiInfoDialogBox(
            message=_("for the table of {}, the score is {}").format(name, self.score),
    ...
    def message(self, message_type):
        AudioPlayer().file_play(
            BRAILLE_LEARNING_APP_FOLDER + "{}.wav".format(message_type), 0
        )
        ...
        if not done:
            ...
            if self.game and Settings().data["braille_learning"]["write_all"]:
                if key_id == Keyboard.KeyId.KEY_CARET_LEFT and self.position > 1:
                    self.position -= 1
                    self.set_data_line()
                elif key_id == Keyboard.KeyId.KEY_CARET_RIGHT and self.position <= len(
                    self.my_expression
                ):
                    self.position += 1
                    self.set_data_line()
        ...
        if not done:
            ...
            if self.game:
                if (
                    ...
                    and self.operation.lst_expression[self.expression][
                        self.position - 1
                    ]
                    != character
                ):
                    ...
                    self.message("ok")
                self.my_expression = (
                    self.my_expression[: self.position]
                    + character
                    + self.my_expression[self.position :]
                )
                self.position += 1
                ...
                if not Settings().data["braille_learning"]["write_all"] and len(
                    self.my_expression
                ) == len(self.operation.lst_expression[self.expression]):
        ...
        if not done:
            ...
            if self.game:
                if bramigraph == Keyboard.BrailleFunction.BRAMIGRAPH_SIMPLE_SPACE:
                    ...
                    self.my_expression = (
                        self.my_expression[: self.position - 2]
                        + self.my_expression[self.position :]
                    )
                    self.position -= 1
                    self.set_data_line()
    ...
    def input_interactive(self, modifier, position, key_type) -> bool:
        ...
        if not done:
            ...
            if (
                position > len(self.expression) + 1
                and position <= len(self.expression) + 1 + len(self.my_expression) + 1
                and Settings().data["braille_learning"]["write_all"]
            ):
                self.position = position - len(self.expression) - 1
                self.set_data_line()
            elif position <= len(self.expression) and Settings().data[
                "braille_learning"
            ]["use_vocal"] != _("no"):
                speak(self.expression)
            done = True
    ...
    def set_data_line(self):
        ...
        if not self.game:
            ...
            line = "".join(
                ["{}=".format(self.expression), "{}".format(self.my_expression)]
            )
            braille_static = BnoteApp.lou.to_dots_8(line)
