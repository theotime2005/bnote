...
class WriteWordApp(BnoteApp):
    ...
    def __init__(self, put_in_function_queue):
        ...
        self.level_dict = {
            ...
            _("A to J"): "{},{}".format(self._get_dict_letter(ord("a"), ord("j")),
            ...
            _("K to T"): "{},{}".format(self._get_dict_letter(ord("k"), ord("t")),
            ...
            _("A to T"): "{},{}".format(self._get_dict_letter(ord("a"), ord("t")),
                                        self._get_dict_letter(ord("A"), ord("T"))),
            _("A to Z"): "{},{}".format(self._get_dict_letter(ord("a"), ord("z")),
    ...
    def initialize(self):
        ...
        if self.is_english:
            ...
            game = WriteWord(Path(self.get_apps_folder() / Path("braille_learning")),
    ...
    def _exec_test_word(self):
        ...
        if self.word == self.first_word:
            ...
            self._current_dialog = ui.UiInfoDialogBox(message=_("good, you have found the good word!!!"),
        ...
        self._current_dialog = ui.UiInfoDialogBox(
            message=_("warning, it's {}, not {}").format(self.first_word, self.word), action=self._exec_cancel_dialog)
    ...
    def _exec_about(self):
        ...
        self._current_dialog = ui.UiMessageDialogBox(
            name=_("information"),
            message=_("Application {} version {}. {} Theotime Berthod").format(_("word game"), app_version,
                                                                               "Copyright (C)2023"),
            buttons=[
    ...
    def _exec_show_score(self):
        name = self.get_key()
        self._current_dialog = ui.UiInfoDialogBox(message=_("for level {}, the score is {}").format(name, self.score),
    ...
    def message(self, message_type):
        AudioPlayer().file_play(
            str(Path(self.get_apps_folder() / Path("braille_learning/{}.wav".format(message_type)))), 0)
        ...
        if not done:
            ...
            if self.file_unable:
                if position <= int(self.length / 2) - 1:
                    ...
                    self.position = 0
                elif position <= int(self.length) / 2 + len(self.word) and Settings().data['braille_learning'][
                    'write_all']:
