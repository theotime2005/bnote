...
class TimerApp(BnoteApp):
    ...
    def __create_menu(self):
        ...
        return ui.UiMenuBar(
            ...
            menu_item_list=[
                ...
                ui.UiMenuBar(
                    name=_("&marker"),
                    action=self._exec_marker,
                    menu_item_list=[
                        ui.UiMenuItem(
                            name=_("add &marker"),
                            action=self._exec_add_marker,
                            shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                            shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_SIMPLE_SPACE,
                        ),
                        ui.UiMenuItem(
                            name=_("&show markers"), action=self._exec_show_marker
                        ),
                    ],
                ),
                ui.UiMenuItem(
    ...
    def _exec_add_marker(self, temps="", nom=""):
        ...
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiButton(
                    name=_("c&reate"),
                    action=self._exec_valid_add_marker,
    ...
    def _exec_ask_replace(self, temps, nom):
        self._current_dialog = ui.UiMessageDialogBox(
            name=_("warning"),
            message=_(
                "a marker already exists at this location, would you like to replace it?"
            ),
            buttons=[
                ui.UiButton(
                    name=_("&yes"),
                    action=self._exec_valid_ask_replace,
    ...
    def _exec_rename_marker(self):
        ...
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiButton(
                    name=_("&ok"),
                    action=self._exec_valid_rename_marker,
    ...
    def _exec_about(self):
        ...
        self._current_dialog = ui.UiMessageDialogBox(
            name=_("information"),
            message=_(
                "timer application, version {}, copyright (C)2023 Theotime Berthod."
            ).format(self.version),
            buttons=[
    ...
    def _exec_show_text_marker(self):
        kwargs = self._current_dialog.get_values()
        self._current_dialog = ui.UiInfoDialogBox(
            message=self.marker[kwargs["marker"]], action=self._exec_show_marker
        )
    ...
    def _exec_valid_timer(self):
        ...
        try:
            ...
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("you must enter numerical values"),
                action=self._exec_cancel_dialog,
            )
            return
        if not self.hours and not self.minutes and not self.seconds:
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("you cannot start the timer at 0."),
                action=self._exec_cancel_dialog,
            )
            return
        self.mode = "timer"
    ...
    def _exec_end_timer(self):
        ...
        self._current_dialog = ui.UiInfoDialogBox(
            message=_("the time is elapsed!!!"), action=audio.stop
        )
