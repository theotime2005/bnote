...
class TimerApp(BnoteApp):
        ...
        return ui.UiMenuBar(
            ...
            menu_item_list=[
                ui.UiMenuItem(name=_("start/stop"), action=self.start_stop,
                        shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE, shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_SIMPLE_RETURN),
                ui.UiMenuBar(
                    name=_("&marker"), action=self._exec_marker,
                    menu_item_list=[
                        ui.UiMenuItem(name=_("add &marker"), action=self._exec_add_marker,
                                shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE, shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_SIMPLE_SPACE),
                        ui.UiMenuItem(name=_("&show markers"), action=self._exec_show_marker),
                    ]
                ),
                ui.UiMenuItem(name=_("&clear"), action=self._exec_clear,
                        shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE, shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_SIMPLE_BACKSPACE),
    ...
    def _exec_time_rapidly(self):
        self._current_dialog=ui.UiInfoDialogBox(message="{}:{}:{}.{}".format(self.hours,self.minutes,self.seconds,self.centiseconds), action=self._exec_cancel_dialog)
    ...
    def _exec_valid_timer(self):
        ...
        try:
            ...
            self._current_dialog=ui.UiInfoDialogBox(message=_("you must enter numerical values"), action=self._exec_cancel_dialog)
            return
        if not self.hours and not self.minutes and not self.seconds:
            self._current_dialog=ui.UiInfoDialogBox(message=_("you cannot start the timer at 0."), action=self._exec_cancel_dialog)
            return
        self.mode="timer"
