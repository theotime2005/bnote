...
class FileManagerApp(BnoteApp):
    ...
    def __create_trash_file_menu(self):
        return ui.UiMenuBar(
            name=_("&file"),
            menu_item_list=[
                ui.UiMenuItem(name=_("&restore from trash"), action=self._exec_restore_from_trash),
    ...
    def __create_menu(self):
        ...
        if len(FileManagerApp.__list_dir(FileManager.get_usb_flash_drive_path())):
            BnoteApp.set_menu(self, self._file_manager__with_usb_flash_drive_plugged_main_menu)
        else:
    ...
    def _exec_unmount_all_usb_flash_drives(self):
        success = True
        usb_drive_list = FileManagerApp.__list_dir(FileManager.get_usb_flash_drive_path())
    ...
    def _exec_rename(self, **kwargs):
        ...
        self._current_dialog = ui.UiDialogBox(
            name=_("rename"),
            item_list=[
                ui.UiFileEditBox(name=_("&name"), value=("rename_name", rename_name_value)),
    ...
    def _exec_delete(self):
        if str(self.__current_folder).startswith(str(Trash.get_trash_path())):
            ...
            self._current_dialog = ui.UiMessageDialogBox(
                ...
                buttons=[
                    ui.UiButton(name=_("&yes"), action=self._exec_permanent_deletion_yes_dialog),
    ...
    def __delete_files(self):
        ...
        if len(delete_files):
            self._current_dialog = ui.UiInfoDialogBox(message=_("deleting {}% {}").format("   ", ""))
    ...
    def _exec_empty_the_trash(self):
        if str(self.__current_folder).startswith(str(Trash.get_trash_path())):
            self._current_dialog = ui.UiMessageDialogBox(
                ...
                buttons=[
                    ui.UiButton(name=_("&yes"), action=self._exec_empty_the_trash_yes_dialog),
                    ui.UiButton(name=_("&no"), action=self._exec_cancel_dialog),
                ],
                action_cancelable=self._exec_cancel_dialog,
            )
        ...
        if len(self.__selected_files) == 1:
            ...
            if self.__selected_files[0].suffix == ".zip":
                ...
                if unzip_to_folder is None:
                    ...
                    return
                self._current_dialog = ui.UiInfoDialogBox(message=_("extracting {}% {}").format("   ", ""))
    ...
    def _exec_recent_file(self):
        ...
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiButton(name=_("&open"), action=self._exec_open_recent_file, action_param={'lst': lst}),
    ...
    def _exec_open_recent_file(self, lst):
        ...
        for element in lst:
            if element[0] == kwargs['file']:
                ...
                print("mon extention: {}, supportées: {}".format(extention, EditorApp.known_extension()))
    ...
    def _exec_ask_open_editor_recent_file(self):
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiButton(name=_("&open"), action=self._exec_valid_open_editor_recent_file)
            ],
            action_cancelable=self._exec_cancel_dialog,
        )
    ...
    def _exec_eole_search(self, ignore_token_validity=False):
        ...
        if Settings().data['eole']['token'] != "" and Settings().data['eole']['user_name'] != "":
            if EoleCrypto(Settings().data['eole']['user_name']).decrypt(Settings().data['eole']['token']) != "":
                EoleApi().set_access_token(
                    EoleCrypto(Settings().data['eole']['user_name']).decrypt(Settings().data['eole']['token']))
        ...
        if has_valid_token or ignore_token_validity:
            self._current_dialog = EoleSearchDialogBox(self.__is_eole_simplified_search,
    ...
    def _exec_eole_login_dialog(self):
        self._current_dialog = ui.UiDialogBox(
            ...
            action_cancelable=self._exec_cancel_dialog,
            item_list=[
                ui.UiEditBox(name=_("login"), value=("login", Settings().data['eole']['user_name'])),
                ui.UiEditBox(name=_("password"), value=("password", "")),
                ui.UiButton(name=_("&ok"), action=self._exec_eole_grab_token),
                ui.UiButton(name=_("&cancel"), action=self._exec_eole_search_without_access_token_check),
            ]
        )
    ...
    def _exec_eole_grab_token(self):
        if self._current_dialog:
            ...
            success, error_success_message = EoleApi().init_access_token_from_user_password(user_name, password)
            if success:
                Settings().data['eole']['user_name'] = user_name
                Settings().data['eole']['token'] = EoleCrypto(user_name).encrypt(EoleApi().access_token())
                Settings().save()
                ...
                self._current_dialog = ui.UiInfoDialogBox(
                    message=_("unable to grab access token. error : {}").format(error_success_message),
    ...
    def _exec_want_to_download(self):
        ...
        if url_name and filename:
            self.__exec_eole_do_prepare_download(**{'url_name': url_name,
    ...
    def __exec_eole_do_prepare_download(self, **kwargs):
        ...
        if 'url_name' in kwargs and kwargs['url_name'] and 'filename' in kwargs and kwargs['filename']:
            ...
            self.__eole_search__current_dialog = self._current_dialog
            self._current_dialog = ui.UiInfoDialogBox(message=_("download in progress..."))
            # Ask a refresh when self._current_dialog is changed.
            self._put_in_function_queue(FunctionId.FUNCTION_FORCE_REFRESH_DIALOG_IN_DIALOG)

            # Call self.__eole_download_thread_function in a thread.
            x = threading.Thread(target=self.__eole_download_thread_function,
    ...
    def __eole_download_thread_function(url_name, filename, on_end=None):
        ...
        try:
            ...
            if on_end is not None:
                on_end(**{'success': success, 'error_message': error_message, 'filename': filename})

    def __eole_end_of_download(self, *args, **kwargs):
        self._put_in_function_queue(FunctionId.FUNCTION_END_EOLE_DOWNLOAD, *args, **kwargs)
    ...
    def _exec_paste(self):
        if self.__copy_cut_paste_files is not None:
            ...
            if self.__is_cut:
                self._current_dialog = ui.UiInfoDialogBox(message=_("moving {}% {}").format("   ", ""))
                ...
                self._current_dialog = ui.UiInfoDialogBox(message=_("copying {}% {}").format("   ", ""))
    ...
    def _exec_restore(self):
        ...
        if len(backup_files) == 0:
            ...
            self._current_dialog = ui.UiDialogBox(
                ...
                action_cancelable=self._exec_cancel_dialog,
                item_list=[
                    ui.UiFileNameListBox(name=_("&name"), value=("backup_file", backup_files)),
    ...
    def __send_to(self, **kwargs):
        ...
        if send_to_mac_address is not None:
            self._current_dialog = ui.UiInfoDialogBox(message=_("sending {}").format(""))
    ...
    def __zip(self, **kwargs):
        ...
        self._current_dialog = ui.UiInfoDialogBox(message=_("compressing {}% {}").format("   ", ""))
    ...
    def __backup(self, **kwargs):
        ...
        self._current_dialog = ui.UiInfoDialogBox(message=_("backuping {}% {}").format("   ", ""))

        self.__zip_thread = ZipThread(zip_files, FileManager.get_backup_path() / filename,
                                      base_folder=FileManager.get_documents_path(), on_progress=self.__on_progress,
                                      on_end=self.__end_backup, zip_hidden_file=True,
                                      exclude_files=(FileManager.get_documents_path() / CLIP_FILE,
    ...
    def __quick_search_move_call_back(self, text_to_find) -> bool:
        ...
        for index, file in enumerate(self.__files):
            ...
            if file_name.lower().find(text_to_find) == 0 and index > self.__focused_file_index:
                    ...
                    if len(self.__locked_filenames) < MAX_EDITOR_APP:
                        ...
                        if extension == ".brf":
                            ...
                            self._put_in_function_queue(
                                FunctionId.OPEN_THE_FILE_PLEASE, **{'filename': focused_file,
                                                                    'language': "FR",
                                                                    'read_only': read_only
                                                                    })
                            return
    ...
    def __activate_mp3_apps(self, **kwargs):
        if Settings().data['system']['app_mp3'] != 'invisible':
            ...
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("please, switch audio application visible in menu in settings to play audio file."),
                action=self._exec_cancel_dialog,
            )
    ...
    def __import_settings(self):
        ...
        self._current_dialog = ui.UiInfoDialogBox(message=_("imported settings."), action=self.__build_braille_line)
