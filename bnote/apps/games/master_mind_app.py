...
from apps.games.master_mind import MasterMind
...
from debug.colored_log import ColoredLogger, MASTERMIND_APP_LOG
...
class MasterMindApp(BnoteApp):
    ...
    def __init__(self, put_in_function_queue):
        ...
        """
        # Call base class.
        super(MasterMindApp, self).__init__(put_in_function_queue)
        # menu creation.
        self.level_msg = {}
    ...
    def _edit_proposition(self):
        if not self._timer_paused:
            self._current_dialog = UiDialogBox(
                name=_("cursor"),
                item_list=[
                     UiEditBox(name=_("value"),
                               value=('value', self._last_proposition)
                               ),
                     UiButton(name=_("&ok"), action=self._exec_valid_proposition_dialog),
                     UiButton(name=_("&cancel"), action=self._exec_cancel_dialog)
                ],
                action_cancelable=self._exec_cancel_dialog,
            )
