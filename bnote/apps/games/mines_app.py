...
class MinesApp(BnoteApp):
    ...
    def _exec_valid_win_dialog(self):
        if (
            self._high_scores.is_better_score(self._current_level, self._timer_seconds)
            != -1
        ):
    ...
    def _exec_valid_win_name_dialog(self):
        ...
        self._high_scores.add_score(
            self._current_level, self._current_name, self._timer_seconds
        )
    ...
    def __last_pos(self):
        if (self._current_line != self._mines.nb_line() - 1) or (
        ...
        if not self._timer_paused:
            ...
            if res == -1:
                ...
                self._current_dialog = ui.UiInfoDialogBox(
                    message=_("boum !"), action=self._exec_cancel_dialog
                )
                self.__lost_game()
    ...
    def __mark_bomb(self) -> bool:
        ...
        if res == -1:
            # Aie ! A mine is discovered.
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("no bomb here !"), action=self._exec_cancel_dialog
            )
            self.__lost_game()
        return True
