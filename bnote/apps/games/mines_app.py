...
from apps.games.high_scores import HighScores
from apps.games.mines import Mines
...
# Setup the logger for this file
from debug.colored_log import ColoredLogger, MINES_APP_LOG
...
class MinesApp(BnoteApp):
    ...
    def __init__(self, put_in_function_queue):
        ...
        """
        # Call base class.
        super(MinesApp, self).__init__(put_in_function_queue)
        # menu creation.
        self.level_msg = {}
        ...
        return UiMenuBar(
            ...
            menu_item_list=[
                UiMenuBar(
                    name=_("&new"),
                    menu_item_list=[
                        UiMenuItem(name="{}".format(self.level_msg['level_1']), action=self._exec_new_1),
                        UiMenuItem(name="{}".format(self.level_msg['level_2']), action=self._exec_new_2),
                        UiMenuItem(name="{}".format(self.level_msg['level_3']), action=self._exec_new_3),
                    ]),
                UiMenuBar(
                    name=_("&scores"),
                    menu_item_list=[
                        UiMenuBar(
                            name=_("&view"),
                            menu_item_list=[
                                UiMenuItem(name="{}".format(self.level_msg['level_1']), action=self._exec_view_1),
                                UiMenuItem(name="{}".format(self.level_msg['level_2']), action=self._exec_view_2),
                                UiMenuItem(name="{}".format(self.level_msg['level_3']), action=self._exec_view_3),
                            ]),
                        UiMenuItem(name=_("&delete"), action=self._exec_delete_high_scores),
                    ]),
            ],
        )
    ...
    def _exec_delete_high_scores(self):
        ...
        self._current_dialog = UiMessageDialogBox(
            ...
            buttons=[
                UiButton(name=_("&ok"), action=self._exec_valid_delete_scores),
    ...
    def _exec_valid_win_dialog(self):
        if self._high_scores.is_better_score(self._current_level, self._timer_seconds) != -1:
            self._current_dialog = UiDialogBox(
                name=_("new score"),
                item_list=[
                    UiFileEditBox(name=_("&name"), value=('name', self._current_name)),
                    UiButton(name=_("&ok"), action=self._exec_valid_win_name_dialog),
                    UiButton(name=_("&cancel"), action=self._exec_cancel_dialog)
                ],
                action_cancelable=self._exec_cancel_dialog,
            )
    ...
    def __display_scores(self, level):
        self._current_dialog = UiDialogBox(
            name=_("scores"),
            item_list=[
                UiListBox(name=_("&score"), value=('score', self._high_scores.scores_list(level))),
    ...
    def __discover(self) -> bool:
        if not self._timer_paused:
            ...
            if res == -1:
                ...
                self._current_dialog = UiInfoDialogBox(message=_("boum !"),
    ...
    def __win(self):
        ...
        self._current_dialog = UiMessageDialogBox(
            ...
            buttons=[
                UiButton(name=_("&ok"), action=self._exec_valid_win_dialog),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
            ],
            action_cancelable=self._exec_cancel_dialog,
        )
    ...
    def __mark_bomb(self) ->bool:
        ...
        if res == -1:
            # Aie ! A mine is discovered.
            self._current_dialog = UiInfoDialogBox(message=_("no bomb here !"),
                                                   action=self._exec_cancel_dialog)
            self.__lost_game()
        return True
