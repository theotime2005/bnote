    ...
    def _exec_send_translation(self):
        ...
        self._current_dialog = ui.UiDialogBox(
            ...
            item_list=[
                ...
                ui.UiListBox(name=_("language &encoding"), value=("encoding", list(Translate.languages_dict.values())),
                             current_index=0),
    ...
    def _exec_valid_send_translation(self):
        ...
        for language in Translate.languages_dict:
            if language == kwargs['encoding']:
                break
        if id_ and language:
            self._put_in_function_queue(FunctionId.FUNCTION_OPEN_BLUETOOTH,
    ...
    def do_translation(self):
        ...
        if len(self.source_text) >= 500:
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("you cannot translate text with more than 500 characters."), action=self._exec_enter_text)
        try:
            self.cible_text = Translator(self.language_dict[self.language_translate],
                                         self.language_dict[self.base_language]).translate(self.source_text)
            self.is_to_translation = True
            self.set_data_line()
        except requests.ConnectionError:
            self._current_dialog = ui.UiInfoDialogBox(
                message=_("the connection to the translation service failed. Check your connexion and try again."),
                action=self._exec_cancel_dialog)
