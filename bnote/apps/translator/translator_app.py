...
class TranslatorApp(BnoteApp):
    ...
    def __create_menu(self):
        ...
        return ui.UiMenuBar(
            ...
            is_root=True,
            menu_item_list=[
                ui.UiMenuItem(name=_("&chose languages"), action=self._exec_chose_languages),
                ui.UiMenuItem(name=_("&invert languages"), action=self._exec_invers_language),
    ...
    def _exec_enter_text(self):
        self._current_dialog=ui.UiDialogBox(
            name=_("enter text"),
            item_list=[
                ui.UiEditBox(name=_("&text to translate"), value=("text", self.source_text)),
    ...
    def _exec_chose_languages(self):
        ...
        self._current_dialog=ui.UiDialogBox(
            ...
            item_list=[
                ui.UiListBox(name=_("&source"), value=("source", lst_language), current_index=index_from),
                ui.UiListBox(name=_("&target"), value=("target", lst_language), current_index=index_to),
    ...
    def _exec_copy(self):
        ...
        self._current_dialog = ui.UiInfoDialogBox(_("translation copy to the clipboard"),
    ...
    def _exec_send_translation(self):
        ...
        self._current_dialog=ui.UiDialogBox(
            name=_("send text to"),
            item_list=[
                ui.UiListBox(name=_("&device"), value=("device", bluetooth_devices), current_index=0),
                ui.UiListBox(name=_("language &encoding"), value=("encoding", list(Translate.languages_dict.values())), current_index=0),
    ...
    def _exec_valid_send_translation(self):
        ...
        for language in Translate.languages_dict:
            if language==kwargs['encoding']:
                break
        if id_ and language:
            self._put_in_function_queue(FunctionId.FUNCTION_OPEN_BLUETOOTH, **{'device': id_, 'text': self.cible_text, 'encoding': language})
    ...
    def input_command(self, data, modifier, key_id) -> bool:
        ...
        if not done:
            ...
            # TODO to complete
            if self.source_text:
                if key_id==Keyboard.KeyId.KEY_CARET_UP or key_id==Keyboard.KeyId.KEY_CARET_LEFT:
                    self.is_to_translation=False
                    self.set_data_line()
                elif key_id==Keyboard.KeyId.KEY_CARET_DOWN or key_id==Keyboard.KeyId.KEY_CARET_RIGHT:
                    self.is_to_translation=True
                    self.set_data_line()
        return done
        ...
        if not done:
            ...
            if bramigraph==Keyboard.BrailleFunction.BRAMIGRAPH_SIMPLE_RETURN:
                if not self.is_to_translation:
                    ...
                    self._exec_copy()
            elif bramigraph==Keyboard.BrailleFunction.BRAMIGRAPH_TAB or bramigraph==Keyboard.BrailleFunction.BRAMIGRAPH_SHIFT_TAB:
                self.comute_text()
        return done
    ...
    def input_interactive(self, modifier, position, key_type) -> bool:
        ...
        # Pass the command to DialogBox / Menu / BrailleDisplay
        done = super(TranslatorApp, self).input_interactive(modifier, position, key_type)
        if not done:
    ...
    def do_translation(self):
        ...
        if len(self.source_text)>=500:
            self._current_dialog=ui.UiInfoDialogBox(message=_("you cannot translate text with more than 500 characters."), action=self._exec_enter_text)
        try:
            self.cible_text=Translator(self.language_dict[self.language_translate],self.language_dict[self.base_language]).translate(self.source_text)
            self.is_to_translation=True
            self.set_data_line()
        except requests.ConnectionError:
            self._current_dialog=ui.UiInfoDialogBox(message=_("the connection to the translation service failed. Check your connexion and try again."), action=self._exec_cancel_dialog)
    ...
    def set_data_line(self):
        ...
        if self.is_to_translation:
            line=_("Translation ({}): {}").format(self.language_translate,self.cible_text)
        else:
