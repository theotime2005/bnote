...
from apps.fman.file_manager import FileManager, BNOTE_FOLDER
...
from ui.ui_document_list import UiDocumentList
...
from debug.colored_log import ColoredLogger, MP3_APP_LOG
...
class Mp3App(BnoteApp):
    ...
    def __init__(self, put_in_function_queue, **kwargs):
        ...
        """
        # Call base class.
        super(Mp3App, self).__init__(put_in_function_queue)
        ...
        return UiMenuBar(
            ...
            menu_item_list=[
                UiMenuBar(
                    name=_("play &list"),
                    menu_item_list=[
                        UiMenuItem(name=_("&open"), action=self._exec_open_playlist,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='O'),
                        UiMenuItem(name=_("&save"), action=self._exec_save_playlist,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='S'),
                        UiMenuItem(name=_("&delete"), action=self._exec_delete_playlist),
                        UiMenuItem(name=_("delete &all"), action=self._exec_delete_all_playlist),
                    ]),
                UiMenuBar(
                    name=_("item"),
                    menu_item_list=[
                        UiMenuItem(name=_("&add"), action=self._exec_add_item,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='A'),
                        UiMenuItem(name=_("de&lete"), action=self._exec_delete_item),
                        UiMenuItem(name=_("delete &all"), action=self._exec_delete_all_items),
                        UiMenuItem(name=_("to the &beginning"), action=self._exec_item_to_beginning,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='H'),
                        UiMenuItem(name=_("to the &end"), action=self._exec_item_to_end,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='E'),
                        UiMenuItem(name=_("&up"), action=self._exec_item_up,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='U'),
                        UiMenuItem(name=_("&down"), action=self._exec_item_down,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='D'),
                        UiMenuItem(name=_("&properties"), action=self._exec_item_properties),
                    ]),
                UiMenuBar(
                    name=_("&play"),
                    menu_item_list=[
                        UiMenuItem(name=_("&item"), action=self._exec_play_selected,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='I'),
                        UiMenuItem(name=_("&list"), action=self._exec_playlist,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='L'),
                        UiMenuItem(name=_("&next"), action=self._exec_next, is_hide=True,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='F'),
                        UiMenuItem(name=_("pre&vious"), action=self._exec_previous, is_hide=True,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='B'),
                        UiMenuItem(name=_("&pause"), action=self._exec_pause_resume, is_hide=True,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='P'),
                        UiMenuItem(name=_("s&top"), action=self._exec_stop, is_hide=True,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='T'),
                        UiMenuItem(name=_("position"), action=self._exec_position, is_hide=True,
    ...
    def _exec_position(self):
        if AudioPlayer().is_playing():
            self._current_dialog = UiDialogBox(
                name=_("editor"),
                item_list=[
                    UiListBox(name=_("&position"), value=('position_value', [str(i) for i in range(0, 100, 5)])),
                    UiButton(name=_("&ok"), action=self._exec_valid_position_dialog),
    ...
    def _exec_add_item(self):
        self._current_dialog = UiDialogBox(
            name=_("insert file"),
            item_list=[
                UiFileBox(root=FileManager.get_root_path(), suffix_filter=FILE_TYPES),
                UiButton(name=_("&ok"), action=self._exec_choose_file),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog)
            ],
            action_cancelable=self._exec_cancel_dialog,
        )
    ...
    def _exec_save_playlist(self, playlist_name=None):
        ...
        self._current_dialog = UiDialogBox(
            name=_("save"),
            item_list=[
                UiEditBox(name=_('playlist'), value=('playlist_name', playlist_name)),
                UiButton(name=_("&save"), action=self.__exec_valid_save_playlist),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
            ],
            action_cancelable=self._exec_cancel_dialog,
        )
    ...
    def __exec_valid_save_playlist(self):
        ...
        if playlist_name.exists():
            ...
            self._current_dialog = UiMessageDialogBox(
                ...
                buttons=[
                    UiButton(name=_("&yes"), action=self._exec_save_playlist_yes_dialog),
                    UiButton(name=_("&no"), action=self._exec_save_playlist_no_dialog),
                    UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
                ],
                action_cancelable=self._exec_cancel_dialog,
    ...
    def __exec_open_delete_playlist(self, function):
        ...
        self._current_dialog = UiDialogBox(
            ...
            item_list=[
                UiListBox(name=_('playlist'), value=('list_of_playlist', self.__get_list_of_playlist())),
                UiButton(name=action_name, action=self.__exec_valid_open_delete_playlist),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
            ],
            action_cancelable=self._exec_cancel_dialog,
        )
    ...
    def _exec_delete_all_playlist(self):
        self._current_dialog = UiMessageDialogBox(
            ...
            buttons=[
                UiButton(name=_("&ok"), action=self.__exec_valid_delete_all_playlist),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
            ],
            action_cancelable=self._exec_cancel_dialog,
        )
    ...
    def _exec_delete_all_items(self):
        if self.__playlist:
            ...
            self._current_dialog = UiMessageDialogBox(
                ...
                buttons=[
                    UiButton(name=_("&ok"), action=self.__exec_valid_delete_all_items),
                    UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
                ],
                action_cancelable=self._exec_cancel_dialog,
            )
    ...
    def _exec_item_properties(self):
        ...
        self._current_dialog = UiDialogBox(
            ...
            item_list=[
                UiEditBox(name=_('tracknumber'),
                          value=('tracknumber', properties['tracknumber'][0])
                          ),
                UiEditBox(name=_('title'),
                          value=('title', properties['title'][0])
                          ),
                UiEditBox(name=_('artist'),
                          value=('artist', properties['artist'][0])
                          ),
                UiEditBox(name=_('date'),
                          value=('date', properties['date'][0])
                          ),
                UiEditBox(name=_('album'),
                          value=('album', properties['album'][0])
                          ),
                UiButton(name=_("&ok"), action=self._exec_valid_properties_dialog),
                UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
            ],
            action_cancelable=self._exec_cancel_dialog,
        )
    ...
    def __build_braille_line(self):
        ...
        self.__ui_line = UiDocumentList(parent_name=str(self.__playlist_name), parent_action=self._exec_activate_list,
