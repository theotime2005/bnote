...
from apps.media.radio import Radio, RADIO_SERVER_URL

# Set up the logger for this file
from debug.colored_log import ColoredLogger, RADIO_APP_LOG
...
class RadioApp(BnoteApp):
    ...
    def __init__(self, put_in_function_queue):
        ...
        """
        # Call base class.
        super(RadioApp, self).__init__(put_in_function_queue)
        # menu creation.
        ...
        return UiMenuBar(
            ...
            menu_item_list=[
                UiMenuBar(
                    name=_("&radio"),
                    menu_item_list=[
                        UiMenuItem(name=_("&radio by url"), action=self._exec_radio_dialog,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='U'),
                        UiMenuItem(name=_("&delete radio"), action=self.__delete_radio,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_NONE,
                                   shortcut_key=Keyboard.BrailleFunction.BRAMIGRAPH_DELETE),
                        UiMenuItem(name=_("&web update list"), action=self._exec_web_update_list,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='N'),
                        # UiMenuItem(name=_("&local update list"), action=self._exec_local_update_list),
                    ]),
                UiMenuBar(
                    name=_("&play"),
                    menu_item_list=[
                        UiMenuItem(name=_("&start"), action=self.__exec_play_radio,
                                   shortcut_modifier=Keyboard.BrailleModifier.BRAILLE_FLAG_CTRL, shortcut_key='L'),
                        UiMenuItem(name=_("&stop"), action=self._exec_stop,
    ...
    def _exec_radio_dialog(self):
        self._current_dialog = UiDialogBox(
            ...
            item_list=[
                ...
                UiButton(name=_("&play"), action=self._exec_radio_by_url),
                UiButton(name=_("add to &list"), action=self._add_radio_by_url),
    ...
    def _add_radio_by_url(self):
        ...
        if self.dialog_radio_name in self.__radio_list:
            self._current_dialog = UiMessageDialogBox(
                ...
                buttons=[
                    UiButton(name=_("&yes"), action=self.__exec_valid_add_radio),
                    UiButton(name=_("&no"), action=self._exec_radio_dialog),
                    UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
                ],
                action_cancelable=self._exec_cancel_dialog,
            )
    ...
    def __delete_radio(self):
        if self.__radio_list:
            ...
            self._current_dialog = UiMessageDialogBox(
                ...
                buttons=[
                    UiButton(name=_("&ok"), action=self.__exec_valid_delete_radio),
                    UiButton(name=_("&cancel"), action=self._exec_cancel_dialog),
                ],
                action_cancelable=self._exec_cancel_dialog,
            )
