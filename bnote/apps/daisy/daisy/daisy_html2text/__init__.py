...
class HTML2Text(html.parser.HTMLParser):
    ...
    @staticmethod
    def __is_empty_text(text: str) -> bool:
    ...
    def outtextf(self, s: str) -> None:
        ...
        # >>> DP
        self.outtagslist.append(self.text_id)
        self.text_id = None
        # <<< DP
        if s:
            self.lastWasNL = s[-1] == "\n"
    ...
    def finish(self) -> str:
        ...
        outstruct = []
        for index, segtext in enumerate(self.outtextlist):
            if self.unicode_snob:
                nbsp = html.entities.html5["nbsp;"]
            else:
                nbsp = " "
            segtext = segtext.replace("&nbsp_place_holder;", nbsp)
            outstruct.append((self.outtagslist[index], segtext))
        outtext = outstruct
        # <<< DP
