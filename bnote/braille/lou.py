...
class Lou:
    ...
    def __init__(self, langue):
        ...
        table_grade1, table_grade2, table_dots8, table_dots6 = Lou.language_dict.get(
            langue, Lou.language_default
        )
        self.tables = Lou.Tables(
            langue,
            grade1=table_grade1,
            grade2=table_grade2,
            dots=table_dots8,
    ...
    def __text_to_grade1(self, txt, cursor=0):
        ...
        if self.tables.grade1:
            return liblouis.translate(
                [self.path + self.tables.grade1], txt, None, cursor
            )
        else:
    ...
    def text_to_grade2(self, txt, cursor=0):
        ...
        with self.lou_mutex:
            if self.tables.grade2:
                return liblouis.translate(
                    [self.path + self.tables.grade2], txt, None, cursor
                )
            else:
    ...
    def to_stm32_display_statics_dots(
        self, static_text, fixed_dots, braille_display_len
    ) -> bytearray:
    ...
    def convert_to_braille(
        self, braille_type, text, pos=-1, no_grade=False
    ) -> (str, str, int):
        ...
        if no_grade:
            ...
            if braille_type == "grade1":
                if pos >= 0:
                    (text_grade, index1_origin, index_text, pos) = self.text_to_grade1(
                        text, pos
                    )
                else:
                    (text_grade, index1_origin, index_text, pos) = self.text_to_grade1(
                ...
                if pos >= 0:
                    (text_grade, index1_origin, index_text, pos) = self.text_to_grade2(
                        text, pos
                    )
                else:
                    (text_grade, index1_origin, index_text, pos) = self.text_to_grade2(
    ...
    def convert_to_text(
        self, braille_type, braille, pos=-1, no_grade=False
    ) -> (str, int):
        ...
        if no_grade:
            ...
            if braille_type == "grade1":
                if pos >= 0:
                    (text_grade1, index1_origin, index_text, pos) = self.grade1_to_text(
                        braille, pos
                    )
                else:
                    (text_grade1, index1_origin, index_text, pos) = self.grade1_to_text(
                ...
                log.debug(
                    "grade1:<{}>, {}, {}, {}".format(
                        text_grade1, index1_origin, index_text, pos
                    )
                )
                text = text_grade1
            elif braille_type == "grade2":
                if pos >= 0:
                    (text_grade2, index1_origin, index_text, pos) = self.grade2_to_text(
                        braille, pos
                    )
                else:
                    (text_grade2, index1_origin, index_text, pos) = self.grade2_to_text(
                ...
                log.debug(
                    "grade2:<{}>, {}, {}, {}".format(
                        text_grade2, index1_origin, index_text, pos
                    )
                )
                text = text_grade2
            else:
...
def main():
    ...
    text_grade1 = "⢁"
    print(
        "backconvertion:<{}> -> <{}>".format(
            text_grade1, converter.to_text_8(text_grade1)
        )
    )
    return
    ...
    (text_grade2, index1_origin, index_text, pos) = converter.text_to_grade2(
        "nouveau fichier.txt"
    )
    ...
    print("----- Test grade1 :")
    (text_grade1, index1_origin, index_text, pos) = converter.text_to_grade1(
        "angle: radian"
    )
