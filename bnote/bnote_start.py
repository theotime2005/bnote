...
import apps.edt.edt as edt
import apps.edt.math_opy as math
from debug.colored_log import ColoredLogger
...
from debug.colored_log import ColoredLogger, BNOTE_LOG
...
class BnoteThread(threading.Thread):
        ...
        while self._running:
            try:
                ...
                if self._bluetooth_init and self._internal_init:
                    ...
                    add_remove_channel_data = bluetooth_thread_pool.get_add_remove_channel()
                    if add_remove_channel_data is not None:
                        log.info("add_remove_channel_data={}".format(add_remove_channel_data))
                        (id_, add, name) = (add_remove_channel_data[0], add_remove_channel_data[1],
                                            add_remove_channel_data[2])
                        if add:
                            self._internal.input_function(FunctionId.FUNCTION_APPEND_BLUETOOTH, id=id_, name=name)
                        else:
                            self._internal.input_function(FunctionId.FUNCTION_REMOVE_BLUETOOTH, id=id_, name=name)
                ...
                if braille_device_characteristics.is_init_done() and BnoteApp.lou and not self._internal_init:
                    log.info("init internal")
                    self._internal = Internal(BnoteApp.lou, self._put_in_function_queue,
                ...
                if braille_device_characteristics.is_init_done() and not self._bluetooth_init:
                    log.info("init bt")
                ...
                if self._internal:
                    if not self._function_queue.empty():
                        ...
                        log.info("command from app. args={} kwarg={}".format(args, kwargs))
                ...
                log.critical("crash...")
                if not sys.path[0].startswith("/home/pi"):
    ...
    def _refresh_braille_from_internal(self, force_refresh=False):
        # force_refresh = True
        static_text, static_dots, dynamic_dots = self._internal.get_data_line(force_refresh)
        if static_text:
        ...
        if static_dots:
            ...
            if sys.path[0].startswith("/home/pi"):
                log.warning("<{}>-<{}>-<{}>".format(static_text, static_dots, dynamic_dots))
            else:
                # Launch from dev. system => Trace allways the braille display.
                log.error("<{}>-<{}>-<{}>".format(static_text, static_dots, dynamic_dots))
    ...
    def _braille_key(self, data):
        log.info("Braille key : {} self._internal_mode={} self._bluetooth_init={}".format(data, self._internal_mode,
                                                                                          self._bluetooth_init))
        if self._internal_mode:
    ...
    def _shutdown_pi(self, data):
        ...
        if is_running_on_rpi():
            ...
            log.critical("shutdown not allowed on dev's PC.")

log.info("Start bnote ...")

if is_running_on_rpi():
    # If bnote_start is running from pycharm, stop bnote service.
    if sys.path[0].startswith("/tmp"):
