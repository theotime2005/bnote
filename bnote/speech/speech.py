...
class SpeechManager(metaclass=SingletonMeta):
    ...
    def enable(self):
        if self._speech_thread is None:
            self._speech_thread = SpeechThread(
                self.__voice_events_callback, self.queued_messages_to_speak
            )
    ...
    def available_speech_languages():
        ...
        """
        return list(
            set(PicoWave().supported_languages())
    ...
    def speak(
        ...
        log.info(
            f"{text_to_speak=} {lang_id=} {synthesis=} {voice=} {volume=} {speed=} {purge_before_speak=}"
        )
            ...
            try:
                ...
                for sentence in sentences:
                    ...
                    if text_to_speak:
                        ...
                        if (synthesis == "picotts") and (
                            lang_id
                            in self.wave_converters["pico"].supported_languages()
                        ):
                            self.wave_converters["pico"].convert_to_wave(
                                text_to_speak=sentence,
                                language=lang_id,
                                voice=voice,
                                file_name=WAVE_FILE_PATH,
                                speed=speed,
                            )
                        elif (synthesis == "mbrola") and (
                            lang_id
                            in self.wave_converters["mbrola"].supported_languages()
                        ):
                            self.wave_converters["mbrola"].convert_to_wave(
                                text_to_speak=sentence,
                                language=lang_id,
                                voice=voice,
                                file_name=WAVE_FILE_PATH,
                            ...
                            lang_id
                            in self.wave_converters["espeak"].supported_languages()
                        ):
                            self.wave_converters["espeak"].convert_to_wave(
                                text_to_speak=sentence,
                                language=lang_id,
                                voice=voice,
                                file_name=WAVE_FILE_PATH,
                            ...
                            lang_id
                            in self.wave_converters["cerence"].supported_languages()
                        ):
                            self.wave_converters["cerence"].convert_to_wave(
                                text_to_speak=sentence,
                                language=lang_id,
                                voice=voice,
                                file_name=WAVE_FILE_PATH,
                        ...
                        while (
                            not self._stop_thread_event.is_set()
                            and not media_player.is_playing()
                            and (
                                not media_player.is_playing()
                                and SpeechThread.soundless_text_watchdog > 0
                            )
                        ):
                            time.sleep(0.01)
                            SpeechThread.soundless_text_watchdog -= 0.01
                            log.warning(
                                f"2. {media_player.is_playing()=} {SpeechThread.soundless_text_watchdog=}"
                            )
                        ...
                        while (
                            not (
                                self._stop_thread_event.is_set()
                                or self._stop_speak_event.is_set()
                            )
                ...
                log.debug(
                    f"call speak in {(time.time_ns() - start_time_speak) / 1000000} ms"
                )
