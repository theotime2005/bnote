...
import glob
import os
from debug.colored_log import ColoredLogger, WAVE_LOG
...
class Wave:
    ...
    def convert_to_wave(self, text_to_speak, language, voice, file_name, speed, volume=None):
...
class PicoWave(Wave):
    ...
    def __init__(self):
        super().__init__()
        self.lang_voice_id = {"en_US": (("M", "en-US"),),
                              "en_GB": (("M", "en-GB"),),
                              "fr_FR": (("M", "fr-FR"),),
                              "es_ES": (("M", "es-ES"),),
                              "de_DE": (("M", "de-DE"),),
    ...
    def to_wave(self, text_to_speak, language, voice, file_name, speed, volume):
        if volume is None:
            volume = 120
        process = os.popen("pico2wave -l {} -w {} '{}'".format(
            voice, file_name, self.__effect(text_to_speak, volume=volume, speed=speed)))
        # Wait execution
        process.read()
...
class ESpeakWave(Wave):
    ...
    def to_wave(self, text_to_speak, language, voice, file_name, speed, volume):
        if volume is None:
            volume = 200
        process = os.popen("espeak-ng -s {} -a {} -v {} '{}' -w {} 2>/dev/null".format(
...
class MBrolaWave(Wave):
    ...
    def to_wave(self, text_to_speak, language, voice, file_name, speed, volume):
        if volume is None:
            volume = 200
            process = os.popen("espeak-ng -s {} -a {} -v mb/mb-{} '{}' -w {} 2>/dev/null".format(
                speed, volume, voice, text_to_speak, file_name))
            # Wait execution
            process.read()
...
class CerenceWave(Wave):
    ...
    def __init__(self):
        ...
        self.lang_voice_id = {
            "ar_LB": (("F", "laila"), ("M", "maged"), ("F", "mariam"), ("M", "tarik"),),
            "cs_CZ": (("F", "iveta"), ("F", "zuzana"),),
            ...
            "hr_HR": (("F", "lana"),),
            # "is_IS": (("M", ""),),
            "it_IT": (("F", "alice"), ("F", "emma"), ("M", "luca"), ("F", "paola"),),
            # "lt_LT": (("M", ""),),
            "nb_NO": (("M", "henrik"), ("F", "nora"),),
            "nl_BE": (("F", "ellen"),),
            "nl_NL": (("F", "claire"), ("M", "xander"),),
            "pl_PL": (("F", "ewa"), ("M", "krzysztof"), ("F", "zosia"),),
            "pt_PT": (("F", "catarina"), ("F", "joana"), ("M", "joaquim"),),
            # "ru_RU": (("M", ""),),
            "sl_SI": (("F", "tina"),),
            "sv_SE": (("F", "alva"), ("F", "klara"), ("M", "oskar"),),
            # "af_ZA": (("M", ""),),
    ...
    def to_wave(self, text_to_speak, language, voice, file_name, speed, volume):
        ...
        if self.is_voice_installed(voice):
            ...
            command_line = "prompter_to_wave --useCaseName=\"{}\" --textToSpeak=\"{}\" --audioScenario=file --configDir=\"{}\" --speedRate={}".format(voice, text_to_speak, CerenceWave.PROMPTER_CONFIG_DIR, speed)
