...
from debug.colored_log import ColoredLogger, STM32_THREAD_LOG
...
class Stm32Thread(threading.Thread):
    TIME_OUT_TO_READ = 1
    TIME_OUT_TO_WRITE = 1
    TIME_OUT_TO_CHECK = 0.2

    TIME_BETWEEN_OUTPUT_FRAMES = 0.05
    ...
    def run(self):
        ...
        while not self._stop:
            ...
            if is_running_on_rpi():
                ...
                # Open serial from USB to Serial convertor (development PC).
                self._serial = serial.Serial("/dev/ttyUSB0", 57600, timeout=Stm32Thread.TIME_OUT_TO_READ)
