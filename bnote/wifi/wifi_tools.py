...
class WifiTools:
    ...
    def network_active_connection():
        ...
        try:
            ...
            for device_path in devices_paths:
                ...
                dev_connectivity = WifiTools.__title(
                    ConnectivityState(generic_dev.ip4_connectivity)
                )
                if dev == "wlan0" and dev_type == "Wifi":
                    ...
                    address_data: WifiTools.NetworkManagerAddressData = (
                        ip4_conf.address_data
                    )
                    for inetaddr in address_data:
                        result["address"] = (
                            f'{inetaddr["address"][1]}/{inetaddr["prefix"][1]}'
                        )
                    # Signal strength
    ...
    def network_favorites():
        ...
        for dbus_connection_path in networkmanager_settings.connections:
            ...
            if (
                profile
                and profile.connection
                and profile.connection.connection_type == ConnectionType.WIFI
            ):
                favorite = {
                    "name": profile.connection.connection_id,
